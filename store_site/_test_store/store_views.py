...
API_DOMAIN = "http://54.199.80.255:8080/"
...
def index_view(request):
  ...
  query = {'offset': 0, 'limit': 1024}
  rtn = cli.call("SCAN_STORE", None, query)[0]
  print( "rtn=", rtn)
...
def create_view(request):
  ...
  if request.method == "POST":
    ...
    cli = new_cli()
    out = cli.call("UPDATE_STORE_INFO", None, save_data)
    print( "out = ", out )
...
def edit_view(request):
  ...
  cli = new_cli()
  detail = cli.call("TABLE_FIND_ONE", None, 's_info', {'sid': sid})[0]
  #print ("detail = ", detail)
  if request.method == "POST":
    ...
    out = cli.call("TABLE_UPDATE_ONE", None, 's_info', {'sid': sid}, save_data)
    print( "out = ", out )
    return HttpResponseRedirect(redirect_to)
...
def detail_view(request):
  ...
  #-- detail --
  cli = new_cli()
  s_info = cli.call("TABLE_FIND_ONE", None, 's_info', {'sid': sid})[0]
  q = {
    'name': 's_pay',
    'query': {'store_id__eq': sid}, 
    'order_by': ('-create_time',),
    'limit': 4096,
  }
  rtn = cli.call("TABLE_QUERY", None, q)[0] # -pay.list
  ids = rtn['ids']
  s_pay_list = cli.call('TABLE_GET_ROW', None, 's_pay', ids)[0]
  #print('SCAN_PAY count', len(ids), ids)
  print('SCAN_PAY count', len(ids))
  #s_pay_list.reverse()

  s_cash_list = []
  #s_cash_list = cli.call("LIST_CASH", sid )
  #s_cash_list.reverse()
  #print("s_cash_list = ", s_cash_list)
  
  return TemplateResponse(request, 'store/detail.html', {
    's_info':s_info,
    'pay_list':s_pay_list,
    'cash_list':s_cash_list,
  })
...
def pay_add_view(request):
  ...
  cli = new_cli()
  #get store info 
  store_info = cli.call("TABLE_FIND_ONE", None, 's_info', {'sid': sid})[0]
  ...
  #save order info.
  #rtn = cli.call("UPDATE_PAY_ORDER", None, sid, _id, save_data)
  rtn = cli.call("TABLE_INSERT_ONE", None, 's_pay', save_data)
  print("TABLE_INSERT_ONE = ", rtn)
...
def pay_query_view(request):
  ...
  cli = new_cli()
  store_info = cli.call("TABLE_FIND_ONE", None, 's_info', {'sid': sid})[0]
  s_pid = store_info.get("pid")
  store_key = store_info.get("key")

  order_info = cli.call("TABLE_FIND_ONE", None, 's_pay', {'store_id': sid, 'order_id': oid})[0]
  ...
  api_query_post = pay_query_api(API_URL_PAY_QUERY, s_pid, store_key, order_money, oid, notify_url)
...
def pay_readd_view(request):
  ...
  s_pay_q = {'store_id': sid, 'order_id': oid}

  order_info = cli.call("TABLE_FIND_ONE", None, 's_pay', s_pay_q)[0]
  ...
  if api_post["code"] == 0:
    ...
    #save order info.
    #rtn = cli.call("UPDATE_PAY_ORDER", sid, oid, update_data)
    rtn = cli.call("TABLE_UPDATE_ONE", None, 's_pay', s_pay_q, update_data)
    print("rtn = ", rtn)
